...
func (e *bloomFilterEncoder) insert8(data []uint8) error {
	h := e.hashes[:]
	for i := 0; i < len(data); {
		n := xxhash.MultiSum64Uint8(h, data[i:])
		e.filter.InsertBulk(h[:n:n])
		i += n
	}
	return nil
}
...
func (e *bloomFilterEncoder) insert16(data []uint16) error {
	h := e.hashes[:]
	for i := 0; i < len(data); {
		n := xxhash.MultiSum64Uint16(h, data[i:])
		e.filter.InsertBulk(h[:n:n])
		i += n
	}
	return nil
...
func (e *bloomFilterEncoder) insert32(data []uint32) error {
	h := e.hashes[:]
	for i := 0; i < len(data); {
		n := xxhash.MultiSum64Uint32(h, data[i:])
		e.filter.InsertBulk(h[:n:n])
		i += n
	}
	return nil
...
func (e *bloomFilterEncoder) insert64(data []uint64) error {
	h := e.hashes[:]
	for i := 0; i < len(data); {
		n := xxhash.MultiSum64Uint64(h, data[i:])
		e.filter.InsertBulk(h[:n:n])
		i += n
	}
	return nil
...
func (e *bloomFilterEncoder) insert128(data [][16]byte) error {
	h := e.hashes[:]
	for i := 0; i < len(data); {
		n := xxhash.MultiSum64Uint128(h, data[i:])
		e.filter.InsertBulk(h[:n:n])
		i += n
	}
	return nil
