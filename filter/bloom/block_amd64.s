...
TEXT ·block_insert(SB), NOSPLIT, $0-16
    ...
    // This initial block is a SIMD implementation of the mask function declared
    // in block_default.go and block_optimized.go, it is repeated in block_check
    // as well. For each of the 8 x 32 bits words of the bloom filter block, the
    // operation performed is:
    //
    //      block[i] = 1 << ((x * salt[i]) >> 27)
    //
    // The algorithm works on the even and odd indexes of the array of salt
    // values because the VPMULUDQ instruction used to perform multiplication of
    // the 32 bits input by the salts produces 4 x 64 bits results in each quad
    // word of the destination YMM register. Since there are no dependencies
    // between the odd and even parts (until we merge the results), the cost of
    // the extra instructions can be amortized by CPU pipelining allowing those
