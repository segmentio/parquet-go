...
TEXT ·dictionaryLookupString(SB), NOSPLIT, $0-112
    ...
    MOVW $0x00FF, R13 // mask for VPGATHER
    MOVW $0x5555, R14 // mask for VPEXPAND
    KMOVW R13, K1
    KMOVW R13, K2
    KMOVW R13, K3
    KMOVW R13, K4
    ...
    VPBROADCASTD BX, Y3            // [len(dict)...]
    VPBROADCASTD R11, Y4           // [size...]
    VPMULLD range0n7<>(SB), Y4, Y4 // [0*size,1*size,...]

    MOVQ $4, R15
    VPBROADCASTQ R15, Z5 // [4,4,4,4,4,4,4,4]
    VPBROADCASTQ CX, Z8  // [page...]

    VPXORQ Z6, Z6, Z6
    VPXORQ Z7, Z7, Z7
loopAVX512:
    // Load the next 8 indexes to read from the dictionary. These may come from
    // user input so we must validate them against the size of the dictionary.
    // We perform the validation with a single comparison of the whole YMM
    // register and test the resulting mask to detect whether any of the indexes
    // were out of bounds, in which case we abort immediately.
    VMOVDQU32 (R8)(SI*4), Y0
    VPCMPUD $1, Y3, Y0, K6
    KMOVW K6, R14
    CMPB R14, $0xFF
    JNE indexOutOfBounds

    // There are 8 offsets in the YMM register that we convert to 8 dictionary
    // offsets.
    //
    // Each of the 8 dictionary offsets points at the beginning of a location
    // in the dictionary page where the length of the PLAIN encoded value is
    // written.
    VPGATHERDD (AX)(Y0*4), K1, Y1
    VPGATHERDD (CX)(Y1*1), K2, Y2

    // The lengths are packed in 32 bits words, but we need them in 64 bits
    // words to be written to the output.
    VPEXPANDD Z1, K5, Z6 // offsets
    VPEXPANDD Z2, K5, Z7 // lengths

    VPADDQ Z5, Z6, Z6
    VPADDQ Z8, Z6, Z6

    VPSCATTERDQ Z6, K3, (R10)(Y4*1)
    VPSCATTERDQ Z7, K4, 8(R10)(Y4*1)

    // The masks are cleared by calls to VPGATHER, we must reset them in
    // preparation for the next loop iteration.
    KMOVW R13, K1
    KMOVW R13, K2
    KMOVW R13, K3
    KMOVW R13, K4
